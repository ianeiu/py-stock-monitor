...
![股票浮窗示例](docs/preview.png)

> 实际运行截图：浅色主题浮窗，顶部按钮区（从左到右）为 关闭 × / 频率 5s / 暂停 ⏸ / 添加 ＋ / 仅变动 🔍 / 走势 📈 / **置顶 📌**；行情行显示名称、价格、涨跌幅、排序 ▲▼、删除 🗑；下方信号提示区显示各股票多空打分；底部状态栏显示刷新频率与数据时间。
...
- **迷你走势图（sparkline）**：每行右侧可选显示约 38px 的日 K 收盘价折线（默认关闭，点 📈 开启）。
- **手动排序**：每行右侧 ▲ / ▼ 按钮上下调整顺序，调整后自动持久化到 `stocks.toml`，重启后保持。
- **仅变动提示**：只看有信号变动的股票（默认开启）；点 🔍 在「仅变动 / 全部显示」间切换。
...
| `filter_on` | `true` | 仅变动提示（只看有信号变动的股票）；false = 全部显示 |
| `show_sparkline` | `false` | 迷你走势图开关；true = 启动即显示（或点 📈 临时开） |
...
- **GUI 交互需真机验证**：跨平台通知、暂停 / 频率、暗色、闪烁、手动排序、sparkline 等均需在对应系统实机验证（本环境无法启动 GUI）。
