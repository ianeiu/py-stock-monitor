...
> 实际运行截图：浅色主题浮窗。
> **紧凑视图**（上图）：顶部按钮区（从左到右）为 **⚙️ 设置 → 🔔 显示变动 → ＋ 新增 → {Ns} 频率 → ↺ 刷新**；行情行显示名称、价格、涨跌幅，右侧 ▲▼ 排序与 🗑 删除工具按钮可由设置面板一键隐藏；信号提示区与状态栏在下方，信号区默认只展示有信号变动的股票（300 秒滑动窗口）。**鼠标移出浮窗自动降到 0.35 透明淡出，移回恢复**；**点击行情行（非工具按钮）即复制股票代码到剪贴板**，状态栏短暂提示「已复制 X」。
> **展开视图**（下图）：点击 ⚙️ 展开设置面板（**透明度 / 灰度滑块 + 2×2 纯文字开关（窗口置顶 / 变动消息 / 隐藏排序 / 隐藏删除）+ 底部 ▾ 收起按钮**；面板钉宽 = 主题窗口宽，开关 / 滑块不会撑宽窗口）；点击 ＋ 展开添加面板（显示名输入 + 股票名模糊搜索 + 代码手动输入 + 搜索结果列表 + 添加 / 取消按钮）。两个面板均位于状态栏下方，顶部带分界线与信号提示区对齐。
...
- **界面置顶开关**：默认置顶；由设置面板「窗口置顶」开关切换（header 不再保留置顶按钮，避免冗余）。
- **信号提示显隐开关**：右上角 🔔 / 🔕 按钮切换信号提示区是否显示（运行时态，不落盘；关闭时标题 / 分隔线 / 容器整块隐藏，开启时维持「仅展示有变动」）。
    - **⚙️ 设置面板**：右上角 ⚙️ 打开设置面板，实时拖动调节「透明度 / 灰度」并一键生效、即时持久化到 config.toml（重启保留）；另含 2×2 纯文字开关（窗口置顶 / 变动消息 / 隐藏排序 / 隐藏删除），底部「▾ 收起设置」按钮关闭面板。面板**钉宽 = 主题窗口宽**，开关 / 滑块变化不会撑宽浮窗。
    - **外观高度可定制**：暗色模式（light / dark / auto 跟随系统）、字号、透明度、涨跌幅配色、信号档位配色、灰度去饱和。
- **命令行回看与统计**：`--review` 回看历史信号（支持按代码 / 日期过滤）、`--stats` 当日信号聚合统计、`--no-log` 不写盘。
- **体验增强**：
    - **鼠标淡出 / 恢复**：鼠标移出浮窗自动降到 0.35 透明度，移回恢复基值；透明度滑块变更会同步基值，无需任何额外点击。
    - **点行情行复制代码**：点击行情行（避开 ▲▼ / 🗑 工具按钮）即把股票代码复制到系统剪贴板，状态栏短暂提示「已复制 X」。
    - **异动闪动高亮**：阈值破位 / 支撑压力穿越时，该行情行（含子 Label）背景临时切到主题暖黄色，280ms 后还原，便于肉眼捕捉异动而不打断阅读。
    - **配置热重载**：后台守护线程每 2s 轮询 `stocks.toml` / `config.toml` 的 mtime，外部编辑后浮窗自动重载（增删 / 重排自选 + 应用外观 / 开关），无需重启。自写盘操作内置 `LAST_*_WRITE_T` 时间戳守卫，不会触发自循环。
...
| `topmost` | `true` | 窗口是否始终置顶（always-on-top）；false = 启动不置顶（或在设置面板「窗口置顶」开关切换） |
...
- **GUI 交互需真机验证**：跨平台通知、频率切换、暗色、闪烁、**鼠标淡出 / hover 恢复（macOS overrideredirect 下 Enter / Leave 是否可靠）、点行情行复制是否误触工具按钮、异动闪动强度是否过 / 不足、设置面板展开后是否真正钉宽 = 主题宽、长文案是否换行而非截断**等均需在对应系统实机验证（本环境无 DISPLAY，无法启动 GUI）。
